...
- Add `FileMagic::fromGeneratedContent()` for application-generated bytes whose Symfony-recognized
  MIME type may be trusted for policy and extension selection, with safe fallback to ordinary
  content inspection when MIME is omitted or unmapped.

### Security

- Keep trusted generated content separate from untrusted uploads, HTTP responses, Data URIs, and
  ordinary `fromContent()` MIME hints.
